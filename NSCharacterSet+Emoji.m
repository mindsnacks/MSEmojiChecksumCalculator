...
static NSString *emojis = nil;

+ (NSString *)emojiString
{
    if (emojis == nil) {
        emojis = [[NSString alloc] initWithData:[NSData dataWithBytes:emoji6
                                                               length:sizeof(emoji6)]
                                       encoding:NSUTF16StringEncoding];
        emojis = [[emojis stringByAppendingString:@"\u20e3"] retain];
    }
...
+ (NSUInteger)numChars
{
    return sizeof(emoji6) / (2*sizeof(const char));
}

+ (NSString *)emojiCharForByte:(char)character
{
    NSUInteger idx = (NSUInteger)character %  [[self emojiString] length];
    NSRange emojiRange = [[self emojiString] rangeOfComposedCharacterSequenceAtIndex:idx];
    return [[self emojiString] substringWithRange:emojiRange];
}
